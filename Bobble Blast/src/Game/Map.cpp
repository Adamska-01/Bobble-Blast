...
Map::Map(bool twoPlayers)
{
	...
	background = new Background(texturesID::BACKGROUND, Vector2(0, ScreenConstants::SCREENHEIGHT), true);
};
...
void Map::AddUI()
{
	for (auto row = 0; row < height; row++)
	{
		for (auto column = 0; column < width; column++)
		{
			...
			{
			case 'S': 
				UI.push_back(new UIText(texturesID::P1_SCORE, Fonts::BOBBLE, " "));
			...
			case 's': 
				UI.push_back(new UIText(texturesID::P2_SCORE, Fonts::BOBBLE, " "));
